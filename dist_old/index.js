...
// server/routes/fichas.ts
import { Router } from "express";

// server/db/postgres.ts
import { Pool } from "pg";
var RENDER_DATABASE_URL = "postgresql://dashboard_captacion_db_user:zCQusHpUln7PINbsqKY3uedDk1tOjcBi@dpg-d54jlsdactks73agf7b0-a.frankfurt-postgres.render.com/dashboard_captacion_db";
var pool = new Pool({
...
var postgres_default = pool;

// server/routes/fichas.ts
var router2 = Router();
router2.get("/", async (req, res) => {
  try {
    const fichasResult = await postgres_default.query(`
      SELECT * FROM fichas 
      ...
      ORDER BY 
        ...
        fecha_creacion DESC
    `);
    const statsResult = await postgres_default.query(`
      SELECT 
        COUNT(*)::int as total,
...
router2.get("/pendientes", async (req, res) => {
  try {
    const result = await postgres_default.query(`
      SELECT * FROM fichas 
      WHERE estado = 'pendiente' 
      ORDER BY 
        CASE prioridad 
...
router2.post("/:id/contactar", async (req, res) => {
  try {
    const { id } = req.params;
    const result = await postgres_default.query(
      `UPDATE fichas 
...
router2.post("/:id/descartar", async (req, res) => {
  try {
    const { id } = req.params;
    const result = await postgres_default.query(
      `UPDATE fichas 
...
async function startServer() {
  ...
  app.use("/api/fichas", fichas_default);
  app.use(
    "/api/trpc",
